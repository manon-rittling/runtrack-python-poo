...
class JoueurHumain(Joueur):
    ...
    def prendre_carte(self, paquet):
        ...
        while nouvelle_carte in self.main:
            print("Carte déjà dans la main. Piocher une nouvelle carte.")
            nouvelle_carte = paquet.pop()

        # Ajouter la nouvelle carte à la main
        self.main.append(nouvelle_carte)
        self.afficher_main()
        
        # Si la carte est un As, laisser le joueur choisir sa valeur
        if nouvelle_carte.valeur == 1:
            nouvelle_carte.valeur = self.choisir_valeur_as()
